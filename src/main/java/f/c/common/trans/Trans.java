...
public class Trans {
    private static final ILoggerService log = LoggerService.getLog(Trans.class);
    ...
    static void _begain(int level) throws Exception {
        ...
        if (null == tn) {
            ...
            count.set(0);
            if (DEBUG)
                log.debug("Start New Transaction id={}, level={}", tn.getId(), level);
        } else {
            if (DEBUG)
                log.debug("Attach Transaction    id={}, level={}", tn.getId(), level);
        }
        int tCount = count.get() + 1;
        count.set(tCount);
        //if (DEBUG)
        //    log.debug("trans_begain: {}", tCount);
    ...
    static void _commit() throws Exception {
        ...
        if (count.get() == 0) {
            ...
            if (DEBUG)
                log.debug("Transaction delay Commit id={}, count={}", tn.getId(), count.get());
        }
    }
    ...
    static void _depose() {
        if (count.get() == 0)
            try {
                if (DEBUG)
                    log.debug("Transaction depose id={}, count={}", trans.get().getId(), count.get());
                trans.get().close();
            }
            catch (Throwable e) {
                throw e;
    ...
    static void _rollback(Integer num) {
        ...
        if (count.get() == 0) {
            if (DEBUG)
                log.debug("Transaction rollback id={}, count={}", trans.get().getId(), num);
            trans.get().rollback();
        } else {
            if (DEBUG)
                log.debug("Transaction delay rollback id={}, count={}", trans.get().getId(), num);
        }
    }
